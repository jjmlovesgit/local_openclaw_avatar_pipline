...
class Wav2VecModel(Wav2Vec2Model):
    ...
    def forward(
        ...
        """
        # Check if we can set output_attentions safely
        try:
            # For sdpa, we can't set output_attentions to True
            # So we'll check if it's supported
            if hasattr(self.config, 'attn_implementation') and self.config.attn_implementation == "sdpa":
                # Don't try to set output_attentions for sdpa
                pass
            else:
                self.config.output_attentions = True
        except (ValueError, AttributeError):
            # If setting fails, just continue without it
            pass
